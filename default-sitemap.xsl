...
>
	...
	<div id="content-head">
		...
		<p>Erstellt von <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank">All in One SEO</a>, dies ist eine XML-Sitemap, die von Suchmaschinen wie Google oder Bing verwendet wird.</p>
		<p>
			Weitere Informationen zu XML-Sitemaps findest du unter <a href="https://www.sitemaps.org/" target="_blank" rel="noreferrer noopener">sitemaps.org</a>.		</p>
		<xsl:if test="$amountOfURLs &gt; 0">
			<p>
				<xsl:choose>
					<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">
						Diese Sitemap enthält						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
					...
					<xsl:otherwise>
						Diese Sitemap-Indexdatei enthält						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								Sitemap							</xsl:when>
							<xsl:otherwise>
				...
				und wurde am 2.10.2023 um 21:47 erstellt			</p>
		</xsl:if>
	</div>
</xsl:template>
	...
	<xsl:variable name="priorityLabel">
		<xsl:choose>
			<xsl:when test="$priorityLevel = 'low'">Niedrig</xsl:when>
			<xsl:when test="$priorityLevel = 'medium'">Mittel</xsl:when>
			<xsl:when test="$priorityLevel = 'high'">Hoch</xsl:when>
		</xsl:choose>
	</xsl:variable>
	...
	<a href="/"><span>Zu Hause</span></a>
		...
		<h2 class="empty-sitemap__title">
			Hoppla!			<br />
			Hier gibt es keine Beiträge		</h2>
